

<div align="center">

<img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&weight=600&size=18&duration=3000&pause=1000&color=FBBF24&center=true&vCenter=true&width=620&lines=Web+Developer+%7C+Competitive+Programmer;Full-Stack+Architect+%7C+AI%2FML+Enthusiast;Building+Scalable+Digital+Ecosystems" alt="Typing SVG" />

</div>

<br>



---

## About Section

<p align="justify">
I am <b>Madhav Vashistha</b>, a Computer Science and Engineering undergraduate with a strong foundation in software engineering, website architecture, and competitive programming. My technical expertise spans C/C++, Python, JavaScript, and modern web frameworks like React, Next.js, and Node.js. Driven by a product-engineering mindset, I specialize in building responsive web applications, secure cloud integrations, and robust automated systems. I am deeply passionate about algorithmic problem-solving, artificial intelligence, and open-source development.
</p>

*   **Primary Focus:** Frontend Development, Full-Stack Architecture, and Competitive Programming.
*   **Engineering Philosophy:** Clean UI/UX design, high performance, modular code structure, and robust security.
*   **Open To:** Software Engineering Opportunities, Open-Source Collaborations, and Advanced Technical Ventures.

---

##  Tech Stack Section

<div align="center">

### Languages & Core
<img src="https://skillicons.dev/icons?i=cpp,c,python,javascript,typescript,html,css" />

### Frontend & UI
<img src="https://skillicons.dev/icons?i=react,nextjs,tailwind,bootstrap,vite,redux" />

### Backend & Databases
<img src="https://skillicons.dev/icons?i=nodejs,express,laravel,php,mysql,postgresql,firebase" />

### Cloud, DevOps & Tooling
<img src="https://skillicons.dev/icons?i=git,github,docker,vscode,postman" />

</div>

---

## 🧠 AI / ML Expertise Section

| Domain | Proficiency | Details |
| :--- | :---: | :--- |
| **Machine Learning Foundations** | Intermediate | Utilizing Python, NumPy, and Scikit-Learn for data manipulation and predictive modeling. |
| **Cloud AI Integration** | Certified | AWS Certified AI Practitioner implementing cloud-based intelligent workflows. |
| **Data Processing & Analytics** | Intermediate | Building automated data pipelines and structured preprocessing workflows for backend integrations. |
